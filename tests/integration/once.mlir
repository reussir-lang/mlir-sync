...
// STD-DAG: func.func private @mlir_sync_call_once_slow_path_prologue(!ptr.ptr<#ptr.generic_space>) -> i1 attributes {{[{](CConv = #llvm\.cconv<preserve_mostcc>, )?}}no_inline, passthrough = ["cold", "nounwind", "noinline"]}
// STD-DAG: func.func private @mlir_sync_call_once_slow_path_epilogue(!ptr.ptr<#ptr.generic_space>) attributes {{[{](CConv = #llvm\.cconv<preserve_mostcc>, )?}}no_inline, passthrough = ["cold", "nounwind", "noinline"]}
...
// STD: %[[EXECUTE:.+]] = {{(func\.)?call}} @mlir_sync_call_once_slow_path_prologue(%[[PTR]]){{( \{CConv = #llvm\.cconv<preserve_mostcc>\})?}} : (!ptr.ptr<#ptr.generic_space>) -> i1
// STD: scf.if %[[EXECUTE]] {
// STD: {{(func\.)?call}} @mlir_sync_call_once_slow_path_epilogue(%[[PTR]]){{( \{CConv = #llvm\.cconv<preserve_mostcc>\})?}} : (!ptr.ptr<#ptr.generic_space>) -> ()
// STD: }
// STD: }
...
// STD: %[[RUN2:.+]] = {{(func\.)?call}} @mlir_sync_call_once_slow_path_prologue(%[[PTR2]]){{( \{CConv = #llvm\.cconv<preserve_mostcc>\})?}} : (!ptr.ptr<#ptr.generic_space>) -> i1
// STD: scf.if %[[RUN2]] {
// STD: memref.load
// STD: memref.store
// STD: {{(func\.)?call}} @mlir_sync_call_once_slow_path_epilogue(%[[PTR2]]){{( \{CConv = #llvm\.cconv<preserve_mostcc>\})?}} : (!ptr.ptr<#ptr.generic_space>) -> ()
// STD: }
// STD: }

// LOWER-DAG: llvm.func{{( preserve_mostcc)?}} @mlir_sync_call_once_slow_path_prologue(!llvm.ptr) -> i1 attributes {passthrough = ["cold", "nounwind", "noinline"], sym_visibility = "private"}
// LOWER-DAG: llvm.func{{( preserve_mostcc)?}} @mlir_sync_call_once_slow_path_epilogue(!llvm.ptr) attributes {passthrough = ["cold", "nounwind", "noinline"], sym_visibility = "private"}
// LOWER-LABEL: llvm.func @execute_marks_complete() -> i1 {
// LOWER: llvm.store
// LOWER: llvm.load
// LOWER: llvm.icmp
// LOWER: llvm.call{{( preserve_mostcc)?}} @mlir_sync_call_once_slow_path_prologue
// LOWER: llvm.call{{( preserve_mostcc)?}} @mlir_sync_call_once_slow_path_epilogue
// LOWER: llvm.return
